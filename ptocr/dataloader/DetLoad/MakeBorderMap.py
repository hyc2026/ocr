...
class MakeBorderMap:
    ...
    def process(self, img, polys, dontcare):
        ...
        # cv2.imshow("", thresh_map)
        # cv2.waitKey(0)
        '''cv2.imshow("output", img)
        cv2.waitKey(5000)
        cv2.imshow("output", thresh_map)
        cv2.waitKey(5000)
        cv2.imshow("output", thresh_mask)
        cv2.waitKey(5000)'''
    ...
    def draw_border_map(self, polygon, canvas, mask):
        ...
        padded_polygon = np.array(padding.Execute(distance)[0])  # 向外扩distance
        cv2.fillPoly(mask, [padded_polygon.astype(np.int32)], 1.0)
        # cv2.fillPoly(mask, [polygon.astype(np.int32)], 0.5)
        # cv2.imshow("", mask)
        # cv2.waitKey(0)
        ...
        # print(polygon)
        # linspace用于生成等差数列，broadcast_to用于把(1, width)复制height行
