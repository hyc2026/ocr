...
def get_img(ori_imgs, config):
    ...
    for ori_img in ori_imgs:
        img, scale = resize_image_batch(ori_img, config['base']['algorithm'], config['testload']['test_size'],
                                        add_padding=config['testload']['add_padding'])
...
class TestProgram():
    ...
    def infer_img(self, ori_imgs):
        ...
        if (self.config['base']['algorithm'] == 'SAST'):
            ...
            scales = [(scale[0], scale[1], ori_imgs.shape[0], ori_imgs.shape[1]) for scale in scales]
...
def InferOneImg(bin, img, image_name, save_path):
    ...
    for i in range(len(bbox_batch)):
        img_show = img[i].copy()
        with open(os.path.join(save_path, 'result_txt', 'res_' + image_name[i] + '.txt'), 'w+',
            ...
            cv2.imwrite(os.path.join(save_path, 'result_img', 'res_' + image_name[i] + '.jpg'), img_show)
...
def InferImage(config):
    ...
    if os.path.isdir(path):
        ...
        for i in range(len(batch_imgs)):
            if i % 10 == 0:
                print("speed of progress --- ", i, "/", len(batch_imgs), " ---")
            # bar.update(1)
...
def InferanImage():
    config = {'base': {'gpu_id': '2', 'algorithm': 'DB', 'pretrained': True, 'in_channels': [256, 512, 1024, 2048], 'inner_channels': 256, 'k': 50, 'adaptive': True, 'crop_shape': [640, 640], 'shrink_ratio': 0.4, 'n_epoch': 1200, 'start_val': 400, 'show_step': 20, 'checkpoints': './checkpoint', 'save_epoch': 100, 'restore': False, 'restore_file': './checkpoint/ag_DB_bb_resnet50_he_DB_Head_bs_8_ep_1200/DB_best.pth.tar'}, 'backbone': {'function': 'ptocr.model.backbone.det_resnet,resnet50'}, 'head': {'function': 'ptocr.model.head.det_DBHead,DB_Head'}, 'segout': {'function': 'ptocr.model.segout.det_DB_segout,SegDetector'}, 'architectures': {'model_function': 'ptocr.model.architectures.det_model,DetModel', 'loss_function': 'ptocr.model.architectures.det_model,DetLoss'}, 'loss': {'function': 'ptocr.model.loss.db_loss,DBLoss', 'l1_scale': 10, 'bce_scale': 1}, 'optimizer': {'function': 'ptocr.optimizer,SGDDecay', 'base_lr': 0.002, 'momentum': 0.99, 'weight_decay': 0.0005}, 'optimizer_decay': {'function': 'ptocr.optimizer,adjust_learning_rate_poly', 'factor': 0.9}, 'trainload': {'function': 'ptocr.dataloader.DetLoad.DBProcess,DBProcessTrain', 'train_file': './dataset/train_list.txt', 'num_workers': 10, 'batch_size': 8}, 'testload': {'function': 'ptocr.dataloader.DetLoad.DBProcess,DBProcessTest', 'test_file': './dataset/test_list.txt', 'test_gt_path': './dataset/det_test_gt', 'test_size': 1536, 'stride': 32, 'num_workers': 5, 'batch_size': 2, 'add_padding': True}, 'postprocess': {'function': 'ptocr.postprocess.DBpostprocess,DBPostProcess', 'is_poly': False, 'thresh': 0.5, 'box_thresh': 0.6, 'max_candidates': 1000, 'unclip_ratio': 2, 'min_size': 3}, 'infer': {'model_path': './checkpoint/ag_DB_bb_resnet50_he_DB_Head_bs_8_ep_1200/DB_best.pth.tar', 'path': './dataset/det_test_image/', 'save_path': './result', 'onnx_path': None, 'trt_path': None}}
    config['infer']['model_path'] = "./checkpoint/my_ag_DB_bb_resnet50_he_DB_Head_bs_8_ep_1100_train_log/DB_900.pth.tar"
    config['infer']['path'] = './result/static/'
    config['infer']['save_path'] = "./result"
    config['testload']['add_padding'] = True
    config['testload']['test_size'] = 1536
    config['testload']['batch_size'] = 2
    path = config['infer']['path']
    save_path = config['infer']['save_path']
    test_bin = TestProgram(config)
    save_path = os.path.abspath(save_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--truth_path', nargs='?', type=str, default=None)
    args = parser.parse_args()
    config = config_load(args)
    os.environ["CUDA_VISIBLE_DEVICES"] = config['base']['gpu_id']
    InferImage(config)
    result_dict = cal_recall_precison_f1(getattr(args, 'truth_path'),
                                         os.path.join(config['infer']['save_path'], 'result_txt'))
    print(result_dict)
    # shutil.rmtree(config['infer']['save_path'], True)
