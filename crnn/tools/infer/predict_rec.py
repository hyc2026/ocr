...
def main(args):
    ...
    with open("res.txt", "w") as f:
        for ino in range(len(img_list)):
            ...
            f.write(valid_image_file_list[ino].split('/')[-1])
            f.write(" ")
            f.write(rec_res[ino][0])
            f.write("\n")
        # print(valid_image_file_list[ino].split('/')[-1], rec_res[ino][0])
    print("Total predict time for {} images, cost: {:.3f}".format(len(img_list), predict_time))
...
def rec():
    ...
    try:
        ...
        exit()
    p = os.path.abspath("")
    with open(os.path.join(p, "result", "res.txt"), "w") as f:
        for ino in range(len(img_list)):
            f.write(str(rec_res[ino][0]))
            f.write("\n")
    print("Total predict time for {} images, cost: {:.3f}".format(len(img_list), predict_time))
